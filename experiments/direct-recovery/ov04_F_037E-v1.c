...
recovered()
{
    ...
    slot->value = 0;
    for (i = 1; i < 36; ++i) {
        if (G_h01_470A[i].value)
            value = G_h01_470A[i].value;
        else {
again:
            value = (unsigned)F_h00_463E() % 37 + 1;
            loot = G_h01_4732;
            for (probe = 1; loot->value != value && probe < 35; ++probe)
                ++loot;
            if (probe >= 35)
                G_h01_470A[i].value = value;
            else {
                ++value;
                if (value >= 37)
                    value = 1;
        ...
        slot = G_h01_4C82 + i;
        if (value == 9)
            G_h01_4705 = buffer[108];
        --value;
        result = F_h00_291E(value, 0L);
